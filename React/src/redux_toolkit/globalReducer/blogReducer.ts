...
export const environment =
  process.env.REACT_APP_API === 'https://api.syncremote.co/api/v1'
    ? 'https://syncremote.co'
    : 'https://dev.syncremote.co'

type BLOG_STATE = {
  blogState: number
  allBlogState: number
  changeStatusState: number
  singleBlogState: number
  blogs: any[]
  currentBlog?: Blog
}
...
export const editBlog: any = createAsyncThunk('editBlog', async (body: any) => {
  const result = await fetch5(`${reactAppBaseurl}/blog/${body?.id}`, body?.data)
  if (result.success === true) {
    blogRes.success = true
    blogRes.message = result.message
  } else {
...
const blogSlice: any = createSlice({
  ...
  reducers: {
    ...
    resetBlogs(state = initialState) {
      state.blogs = []
    },
    setBlogs(state = initialState, action) {
      state.blogs = action.payload
    },
    setCurrentBlog(state = initialState, action) {
